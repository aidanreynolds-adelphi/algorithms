"""MLP hyperparameter grid search for the obesity dataset."""
...
MLP_PARAM_GRID: dict[str, object] = {
    "hidden_layer_sizes": [
        (256, 128),
        (256, 128, 64),
        (512, 256),
...
def run_mlp_grid_search(df: pd.DataFrame) -> GridSearchCV:
    """
    Run GridSearchCV to find best MLP hyperparameters; write results to report folder.
    ...
    report_path = REPORT_DIR / "nn_gridsearch_report.txt"
    lines = [
        "MLP GridSearchCV results (obesity dataset)",
        "=" * 50,
        f"CV folds: {CV_FOLDS}",
    ...
    cv_path = REPORT_DIR / "nn_gridsearch_cv_results.csv"
...
def main() -> None:
    """Run GridSearchCV for MLP hyperparameters and write results to report folder."""
    df = load_obesity_data()
    grid = run_mlp_grid_search(df)
